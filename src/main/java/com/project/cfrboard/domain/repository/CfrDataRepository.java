...
public interface CfrDataRepository extends JpaRepository<CfrData, Long> {
    ...
    @Query("select new com.project.cfrboard.domain.dto.CfrDataDto(c.value, c.confidence, c.createDate) from CfrData c where c.member = :member order by c.createDate desc")
    List<CfrDataDto> findCfrDataDtoList(@Param("member") Member member);
